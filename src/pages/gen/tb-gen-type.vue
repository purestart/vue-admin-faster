...
<template>
  <div class="page-gen--list">
    ...
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item v-if="$hasPermission('gen:tb-gen-type:query')">
          <el-button size="small" @click="queryForm()">查询</el-button>
      </el-form-item>
        
      <el-form-item v-if="$hasPermission('gen:tb-gen-type:save')">
          <el-button size="small"  type="primary" @click="addOrUpdateHandle()">新增</el-button>
      </el-form-item>
         
      <el-form-item v-if="$hasPermission('gen:tb-gen-type:delete')">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="addOrUpdateHandle(scope.row.id)"  v-if="$hasPermission('gen:tb-gen-type:edit')">修改</el-button>
          <el-button type="text" size="small" @click="deleteHandle(scope.row.id)"  v-if="$hasPermission('gen:tb-gen-type:delete')">删除</el-button>
...
<script>
  import AddOrUpdate from './tb-gen-type-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定进行${id ? '删除' : '批量删除'}操作?`, '提示', {
          ...
          this.$axios.post(
              `${this.$baseUrl}/gen/tb-gen-type/delete`,
               {ids}
           ).then(({data}) => {
            if (data && data.success === 1) {
              this.getDataList()

...
<template>
  <div class="page-project--list">
                ...
                <div class="input-box">
                    <div class="item-row">
                          ...
                          <div class="item">
                            ...
                            <el-input class="item-input" size="small" v-model="dataForm.dealer_name" placeholder="经销商名称" clearable></el-input>
                          </div>
                          
                          <div class="item">
                            <span class="label">项目名称</span>
                            <el-input class="item-input" size="small" v-model="dataForm.project_name" placeholder="项目名称" clearable></el-input>
                          </div>
    ...
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item v-if="$hasPermission('project:project-ctrl:query')">
          <el-button size="small" @click="queryForm()">查询</el-button>
      </el-form-item>
        
      <el-form-item v-if="$hasPermission('project:project-ctrl:save')">
          <el-button size="small"  type="primary" @click="addOrUpdateHandle()">新增</el-button>
      </el-form-item>
         
      <el-form-item v-if="$hasPermission('project:project-ctrl:delete')">
          <el-button size="small"  type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
         
      <el-form-item v-if="$hasPermission('project:project-ctrl:import')">
        <el-button size="small"  type="info" @click="exportHandle()">导入</el-button>
      </el-form-item>
           
      <el-form-item v-if="$hasPermission('project:project-ctrl:export')">
        <el-button size="small" type="success" @click="exportHandle()">导出</el-button>
      </el-form-item>
        
      <el-form-item  v-if="$hasPermission('project:project-ctrl:query')"  style="float:right">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="addOrUpdateHandle(scope.row.id)"  v-if="$hasPermission('project:project-ctrl:edit')">修改</el-button>
              
          
          <el-button type="text" size="small" @click="deleteHandle(scope.row.id)"  v-if="$hasPermission('project:project-ctrl:delete')">删除</el-button>
...
<script>
  import AddOrUpdate from './project-ctrl-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$axios.get(
          `${this.$baseUrl}/project/project-ctrl/list`,
          {
              params: {
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定进行${id ? '删除' : '批量删除'}操作?`, '提示', {
          ...
          this.$axios.post(
              `${this.$baseUrl}/project/project-ctrl/delete`,
               {ids}
           ).then(({data}) => {
            if (data && data.success === 1) {
              this.getDataList()
